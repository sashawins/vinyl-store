...
// The VinylFunc type is an adapter to allow the use of ordinary
// function as Vinyl mutator.
type VinylFunc func(context.Context, *ent.VinylMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f VinylFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.VinylMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.VinylMutation", m)
}
