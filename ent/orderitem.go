...
type OrderItem struct {
	...
	// VinylID holds the value of the "vinyl_id" field.
	VinylID uuid.UUID `json:"vinyl_id,omitempty"`
...
func (*OrderItem) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case orderitem.FieldID, orderitem.FieldOrderID, orderitem.FieldVinylID:
...
func (oi *OrderItem) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case orderitem.FieldVinylID:
			if value, ok := values[i].(*uuid.UUID); !ok {
				return fmt.Errorf("unexpected type %T for field vinyl_id", values[i])
			} else if value != nil {
				oi.VinylID = *value
			}
		case orderitem.FieldQuantity:
...
func (oi *OrderItem) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("vinyl_id=")
	builder.WriteString(fmt.Sprintf("%v", oi.VinylID))
	builder.WriteString(", ")
