...
func (oic *OrderItemCreate) SetVinylID(u uuid.UUID) *OrderItemCreate {
	oic.mutation.SetVinylID(u)
	return oic
}
...
func (oic *OrderItemCreate) check() error {
	...
	if _, ok := oic.mutation.VinylID(); !ok {
		return &ValidationError{Name: "vinyl_id", err: errors.New(`ent: missing required field "OrderItem.vinyl_id"`)}
	}
	if _, ok := oic.mutation.Quantity(); !ok {
...
func (oic *OrderItemCreate) createSpec() (*OrderItem, *sqlgraph.CreateSpec) {
	...
	if value, ok := oic.mutation.VinylID(); ok {
